<template>
    <v-container>
                ...
                <v-card v-else light elevation="8" raised class="mt-5">
                    ...
                    <v-card-text>
                        <div class="text-right my-3" v-if="!openReplyField">
                            ...
                            <v-btn text color="red darken-2" class="ml-2" @click="confirmThreadDelDial = true">Delete Thread</v-btn>
                        </div>
                        <div class="px-5">
                            <table class="table table-condensed table-borderless">
                                <thead></thead>
        ...
        <v-dialog v-model="confirmThreadDelDial" max-width="480">
            <v-card min-height="150">
                <v-card-title class="subtitle-1 justify-center pt-4 primary white--text">Do you want to delete this feedback thread?</v-card-title>
                <v-card-actions class="mt-8 pb-8 justify-space-around">
                    <v-btn text color="red darken--2" @click="confirmThreadDelDial = false" width="40%">Cancel</v-btn>
                    <v-btn dark color="primary" :loading="isBusy" @click="delFeedbackThread" width="40%">Yes, Delete</span></v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-snackbar v-model="threadDelFail" :timeout="6000" top color="red darken-1 white--text">
            There was an error while trying to delete this feedback thread. Please try again.
            <v-btn text color="white--text" @click="threadDelFail = false">Close</v-btn>
        </v-snackbar>
    </v-container>
</template>
...
<script>
    ...
    methods: {
        ...
        delFeedbackThread(){
            this.isBusy = true
            axios.post(this.api + `/auth-admin/admin_delete_feedback_thread/${this.$route.params.id}`, {}, this.adminHeaders)
            .then((res) => {
                this.isBusy = false
                this.$store.commit('adminDeletedfeedbackThread')
                this.$router.push({name: 'AdminFeedbacks'})
            }).catch(() => {
                this.isBusy = false
                this.threadDelFail = true
            })
        }
    },
    created() {
