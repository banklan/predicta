<template>
    <div>
        <v-card flat>
            ...
            <v-card-actions class="justify-space-around pb-6 mt-5">
                <v-btn text color="red darken-2" width="40%" @click="clearSelects">Cancel</v-btn>
                <v-btn dark color="primary darken-2" width="40%" @click="submit" :loading="isBusy">Save</v-btn>
            </v-card-actions>
        </v-card>
        ...
        <v-snackbar v-model="createSuccess" :timeout="6000" top color="green darken-1 white--text">
            Tip was added to the daily tips successfully.
                ...
                if (isValid) {
                    if(this.select.home !== this.select.away){
                        if(this.select.date !== null && this.select.time !== null){
                            ...
                            this.isBusy = true
                            axios.post(this.api + `/auth-admin/add_tip_to_daily_tips/${this.$route.params.code}`, {
                                tip: this.select
                                ...
                                this.clearSelects()
                                this.$store.commit('tipAddedToDailyTips')
                            })
                        }else{
